...
const adminOrdersEnhanced = require('./admin-orders-enhanced');
...
router.use('/orders-enhanced', adminOrdersEnhanced);
...
// ==================== LOCAL MARKET PAYMENT APPROVALS ====================

// GET /api/admin/local-market-payments - Get local market payment approvals
router.get('/local-market-payments', enhancedErrorHandler(async (req, res) => {
    ...
    try {
        const [approvals] = await db.execute(`
            SELECT 
                ...
                lmo.total_amount,
                lmo.status as order_status,
                lmo.created_at as order_created_at
            FROM admin_approvals aa
            LEFT JOIN users u ON aa.user_id = u.id
            LEFT JOIN local_market_orders lmo ON aa.reference_id = lmo.id
            WHERE aa.type = 'LOCAL_MARKET_PAYMENT' 
            AND aa.status = ?
            ORDER BY aa.created_at DESC
            LIMIT ? OFFSET ?
        `, [status, parseInt(limit), offset]);

        // Parse the data field for each approval
        const processedApprovals = approvals.map(approval => {
            let parsedData = {};
            try {
                parsedData = JSON.parse(approval.data || '{}');
            } catch (e) {
                console.warn('Failed to parse approval data:', e);
            }
        ...
        const [countResult] = await db.execute(`
            SELECT COUNT(*) as total 
            FROM admin_approvals 
            WHERE type = 'LOCAL_MARKET_PAYMENT' AND status = ?
        `, [status]);

        res.json({
            success: true,
            message: 'Local market payment approvals retrieved successfully',
        ...
        console.error('Error fetching local market payment approvals:', error);
        res.status(500).json({
            success: false,
...
// POST /api/admin/local-market-payments/:id/approve - Approve local market payment
router.post('/local-market-payments/:id/approve', enhancedErrorHandler(async (req, res) => {
    const { id } = req.params;
    const { notes } = req.body;
    ...
    const [approvals] = await db.execute(`
        ...
        WHERE id = ? AND type = 'LOCAL_MARKET_PAYMENT' AND status = 'pending'
    `, [id]);
    ...
    const orderId = approval.reference_id;
    ...
    await db.execute(`
        ...
        SET status = 'paid', payment_verified_at = NOW(), payment_verified_by = ?
        WHERE id = ?
    `, [req.user.id, orderId]);
    ...
    `, [orderId, `Payment approved by admin: ${notes || 'No notes'}`, req.user.id]);
    ...
    if (req.app.get('io')) {
        ...
        req.app.get('io').emit('adminNotification', {
            ...
            message: `Local market payment approved for Order #${orderId}`,
            orderId: orderId
        });
    }
...
// POST /api/admin/local-market-payments/:id/reject - Reject local market payment
router.post('/local-market-payments/:id/reject', enhancedErrorHandler(async (req, res) => {
    const { id } = req.params;
    ...
    `, [orderId, `Payment rejected by admin: ${reason}. ${notes || ''}`, req.user.id]);
...
// DELETE /api/admin/files/:id - Delete file or folder
router.delete('/files/:id', enhancedErrorHandler(async (req, res) => {
    const { id } = req.params;
...
router.get('/files/:id/download', enhancedErrorHandler(async (req, res) => {
    const { id } = req.params;
    ...
    res.json({
        ...
        download_url: `/uploads/files/${id}`,
        expires_at: new Date(Date.now() + 60 * 60 * 1000).toISOString() // 1 hour
    });
}));

// POST /api/admin/files/:id/share - Generate share link
router.post('/files/:id/share', enhancedErrorHandler(async (req, res) => {
    const { id } = req.params;
    const { expires_in = 3600, password = null } = req.body; // Default 1 hour

    const shareLink = {
        id: Date.now(),
        file_id: parseInt(id),
        share_token: Math.random().toString(36).substring(2, 15),
        share_url: `${req.protocol}://${req.get('host')}/share/${Math.random().toString(36).substring(2, 15)}`,
        created_at: new Date().toISOString(),
...
// GET /api/admin/files/storage-stats - Get storage statistics
router.get('/files/storage-stats', enhancedErrorHandler(async (req, res) => {
    // Mock storage statistics
    const stats = {
        total_storage: 4 * 1024 * 1024 * 1024, // 4GB
        used_storage: 2.4 * 1024 * 1024 * 1024, // 2.4GB
        available_storage: 1.6 * 1024 * 1024 * 1024, // 1.6GB
        usage_percentage: 60,
        file_types: {
            images: { count: 450, size: 850 * 1024 * 1024 },
            videos: { count: 25, size: 1200 * 1024 * 1024 },
            documents: { count: 230, size: 150 * 1024 * 1024 },
            audio: { count: 45, size: 180 * 1024 * 1024 },
            archives: { count: 12, size: 320 * 1024 * 1024 },
            other: { count: 89, size: 50 * 1024 * 1024 }
        },
        largest_files: [
            { name: 'promotional-video-hd.mp4', size: 156 * 1024 * 1024, path: '/videos/promotional-video-hd.mp4' },
            { name: 'product-catalog.zip', size: 89 * 1024 * 1024, path: '/documents/product-catalog.zip' },
            { name: 'company-presentation.pptx', size: 45 * 1024 * 1024, path: '/documents/company-presentation.pptx' }
        ],
        recent_uploads: [
            { name: 'new-logo.png', size: 256000, uploaded_at: new Date(Date.now() - 30 * 60 * 1000) },
            { name: 'user-manual.pdf', size: 1024000, uploaded_at: new Date(Date.now() - 2 * 60 * 60 * 1000) },
            { name: 'backup-file.zip', size: 5120000, uploaded_at: new Date(Date.now() - 4 * 60 * 60 * 1000) }
        ]
    };
...
// POST /api/admin/files/cleanup - Clean up temporary files
router.post('/files/cleanup', enhancedErrorHandler(async (req, res) => {
    ...
    const cleanupResult = {
        files_deleted: 23,
        space_freed: 156 * 1024 * 1024, // 156MB
        folders_removed: 3,
        cleanup_duration: 2.1 // seconds
    };
...
// GET /api/admin/files/recent-activity - Get recent file activity
router.get('/files/recent-activity', enhancedErrorHandler(async (req, res) => {
    const { limit = 50 } = req.query;
    ...
    const activities = [
        {
            ...
            file_path: '/product-images/product-photo-1.jpg',
        ...
        },
        {
            ...
            file_path: '/documents/old-document.pdf',
            user_name: 'Admin User',
            timestamp: new Date(Date.now() - 45 * 60 * 1000),
            size: 256000
        },
        {
            ...
            old_name: 'terms.pdf',
            file_path: '/documents/updated-terms.pdf',
            user_name: 'Admin User',
        ...
        },
        {
            ...
            file_name: 'new-campaign',
            file_path: '/marketing/new-campaign',
            user_name: 'Admin User',
            timestamp: new Date(Date.now() - 2 * 60 * 60 * 1000)
        },
        {
            ...
            file_path: '/branding/logo-v2.png',
            old_path: '/temp/logo-v2.png',
            user_name: 'Admin User',
