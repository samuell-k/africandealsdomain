...
router.put('/update-status/:orderId', authenticateToken, verifyFastDeliveryAgent, async (req, res) => {
  try {
    ...
    // If delivered, update agent status and calculate commission
    if (status === 'delivered') {
      await pool.query(
...
function calculateCommission(orderTotal) {
  ...
  const maxCommission = 20.00; // Maximum $20 commission
